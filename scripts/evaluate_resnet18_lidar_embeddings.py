...
class LidarPatchDataset:
    ...
    def __getitem__(self, index: int):
        ...
        image = Image.open(record.lidar_path).convert("RGB")
        return self.transform(image), record.label


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate pretrained ResNet-18 embeddings on LiDAR-derived images."
    )
    parser.add_argument(
        "--audit-csv",
        type=Path,
...
def main() -> int:
    ...
    if not records:
        print(f"No usable LiDAR records found in: {args.audit_csv}")
        return 1

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    transform = _build_transform(transforms, args.image_size)
    loader = DataLoader(
        LidarPatchDataset(records, transform),
...
def _evaluate_leave_one_site_out(
    ...
    print("LiDAR ResNet-18 embeddings leave-one-site-out")
